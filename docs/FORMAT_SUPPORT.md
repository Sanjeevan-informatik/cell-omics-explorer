...
5 MiB per text file; 20 MiB total previews; 20 files per batch; 40 session entries. Delimited tables are capped at 10,000 data rows. Structured previews retain up to 5,000 rows for delimited tables and show up to 100 matching rows. Missing values are preserved, never filled with zero. Ready means the table can be inspected, not that all biological semantics are validated.
